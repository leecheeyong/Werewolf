...
class WerewolfGame {
  ...
  toggleLanguage() {
    ...
    this.langToggle.textContent =
      this.currentLang === "zh" ? "English" : "中文";
    this.updateLanguage();
  }
  ...
  updateLobbyPlayers() {
    ...
    this.gameState.players.forEach((player) => {
      ...
      playerDiv.className =
        "flex items-center space-x-3 p-3 bg-gray-700 rounded-lg border border-gray-600";
      playerDiv.innerHTML = `
  ...
  updateHunterRevengePanel() {
    if (
      this.gameState.gameState === "hunter_revenge" &&
      this.gameState.hunterRevenge === this.socket.id
    ) {
      ...
      title.textContent =
        this.currentLang === "zh"
          ? "猎人复仇 - 选择一个玩家带走"
          : "Hunter's Revenge - Choose a player to eliminate";
  ...
  updatePlayersList() {
    ...
    this.gameState.players.forEach((player) => {
      ...
      const eliminatedText =
        this.currentLang === "zh" ? "已淘汰" : "ELIMINATED";
      const analysisBtn =
        (this.playerRole === "seer" || this.playerRole === "doctor") &&
        player.alive &&
  ...
  updateNightActionsPanel() {
    if (this.gameState.gameState === "night") {
      ...
      const alivePlayers = this.gameState.players.filter(
        (p) => p.alive && p.id !== this.socket.id,
      );
      if (this.playerRole === "werewolf") {
        const title =
          this.currentLang === "zh"
            ? "狼人投票 - 选择消灭目标"
            : "Werewolf Vote - Choose Kill Target";
        ...
        if (this.gameState.nightActionsComplete?.werewolfVotes) {
          ...
          votesDiv.className =
            "mb-4 p-3 bg-red-900 rounded border border-red-500";
          const votesTitle = document.createElement("div");
          votesTitle.className =
            "text-red-400 font-semibold game-font text-sm mb-2";
          votesTitle.textContent =
            this.currentLang === "zh" ? "狼人投票情况:" : "Werewolf Votes:";
          votesDiv.appendChild(votesTitle);
          this.gameState.nightActionsComplete.werewolfVotes.forEach(
            ([voterId, targetId]) => {
              const voter = this.gameState.players.find(
                (p) => p.id === voterId,
              );
        ...
        if (myVote) {
          const myTarget = this.gameState.players.find(
            (p) => p.id === myVote[1],
          );
          if (myTarget) {
            const myVoteDiv = document.createElement("div");
            myVoteDiv.className =
              "text-green-400 font-semibold mb-2 game-font text-sm";
            myVoteDiv.textContent =
              this.currentLang === "zh"
                ? `你的投票: ${myTarget.name}`
                : `Your vote: ${myTarget.name}`;
        ...
        const aliveWerewolves = this.gameState.players.filter(
          (p) => p.alive && this.werewolves.includes(p.id),
        ).length;
        const currentVotes =
          this.gameState.nightActionsComplete?.werewolfVotes?.length || 0;
        const progressDiv = document.createElement("div");
        progressDiv.className =
          "mt-4 text-yellow-400 game-font text-sm text-center";
        ...
        const title =
          this.currentLang === "zh" ? "选择查验目标" : "Choose Check Target";
        const titleDiv = document.createElement("div");
        ...
        if (this.seerResult) {
          ...
          resultDiv.className =
            "mt-4 p-3 bg-purple-900 rounded border border-purple-500";
          const resultText =
            this.currentLang === "zh"
        ...
        const title =
          this.currentLang === "zh" ? "选择拯救目标" : "Choose Save Target";
        const titleDiv = document.createElement("div");
        ...
        if (
          this.gameState.nightActionsComplete &&
          this.gameState.nightActionsComplete.werewolfKill
        ) {
          const attackedPlayer = this.gameState.players.find(
            (p) => p.id === this.gameState.eliminatedTonight?.[0],
          );
          if (attackedPlayer) {
        ...
        if (currentPlayerData?.hasAntidote) {
          const antidoteTitle =
            this.currentLang === "zh" ? "使用解药" : "Use Antidote";
          const titleDiv = document.createElement("div");
        ...
        if (currentPlayerData?.hasPoison) {
          const poisonTitle =
            this.currentLang === "zh" ? "使用毒药" : "Use Poison";
          const titleDiv = document.createElement("div");
          titleDiv.className =
            "text-green-400 font-semibold mb-2 mt-4 game-font";
        ...
        if (!currentPlayerData?.hasAntidote && !currentPlayerData?.hasPoison) {
          ...
          noAbilitiesDiv.textContent =
            this.currentLang === "zh"
              ? "你已经用完了所有能力"
              : "You have used all your abilities";
  ...
  updateGameEvents() {
    ...
    if (
      this.gameState.eliminatedTonight &&
      this.gameState.eliminatedTonight.length > 0
    ) {
  ...
  updatePhaseDisplay() {
    ...
    if (this.gameState.gameState === "night") {
      ...
      this.gameHeader.className =
        "bg-gray-800 border-b-2 border-purple-500 p-4";
    } else if (this.gameState.gameState === "hunter_revenge") {
      this.gameScreen.className =
        "min-h-screen bg-gradient-to-br from-orange-900 via-red-900 to-black";
      this.gameHeader.className =
        "bg-gray-800 border-b-2 border-orange-500 p-4";
    } else {
      this.gameScreen.className =
  ...
  showGameOverScreen() {
    ...
    if (this.gameState.winner === "villagers") {
      const winText =
        this.currentLang === "zh" ? "村民获胜!" : "VILLAGERS WIN!";
      const winDesc =
        this.currentLang === "zh"
          ? "正义战胜了邪恶！所有狼人都被消灭了。"
      ...
      this.gameResult.className =
        "title-font text-5xl md:text-7xl mb-8 text-blue-400";
      this.gameResultText.textContent = winDesc;
    } else {
      const winText =
        this.currentLang === "zh" ? "狼人获胜!" : "WEREWOLVES WIN!";
      const winDesc =
        this.currentLang === "zh"
          ? "黑暗降临！狼人成功控制了村庄。"
      ...
      this.gameResult.textContent = winText;
      this.gameResult.className =
        "title-font text-5xl md:text-7xl mb-8 text-red-400";
      this.gameResultText.textContent = winDesc;
    }
