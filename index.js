...
class WerewolfGame {
  ...
  werewolfKill(killerId, targetId) {
    console.log(
      `Werewolf vote attempt: ${killerId} -> ${targetId}, gameState: ${this.gameState}`,
    );
    ...
    const aliveWerewolves = Array.from(this.roles.werewolves).filter(
      (id) => this.players.get(id)?.alive,
    );
  ...
  analyzeMessage(playerId, message) {
    ...
    if (
      ...
      lowerMessage.includes("innocent") ||
      lowerMessage.includes("trust me")
    ) {
  ...
  getNightActionsStatus() {
    ...
    const werewolfVotesComplete = this.werewolfVotes
      ? this.werewolfVotes.size === aliveWerewolves.length
      : false;

    return {
      werewolfKill: werewolfVotesComplete,
      werewolfVotes: this.werewolfVotes
        ? Array.from(this.werewolfVotes.entries())
        : [],
      ...
      witchAntidote: this.roles.witch
        ? !!this.nightActions.witchAntidote
        : true,
    };
  }
  ...
  getPlayerRole(playerId) {
    ...
    const roleInfo = {
      role: player.role,
      werewolves:
        player.role === "werewolf" ? Array.from(this.roles.werewolves) : [],
